...
from cws_globals import (
    UNION, CONFEDERATE, UNION_ARMY_END, NUM_ARMIES, NUM_CITIES,
)
...
def icon(g: 'GameState', from_: int, dest: int, kind: int) -> None:
    ...
    if from_ < 1 or from_ > NUM_CITIES:                    # L16: noshow
        return
    if dest < 0:                                           # L17
        return
    if from_ == 999:                                       # L18
    ...
    x1 = g.cityx[dest] if 0 <= dest <= NUM_CITIES else 0   # L20
    y1 = g.cityy[dest] if 0 <= dest <= NUM_CITIES else 0
...
def showcity(g: 'GameState') -> None:
    ...
    s = g.screen
    for i in range(1, NUM_CITIES + 1):                      # L90
        c = 9                                              # L91: Union blue
...
def tupdate(g: 'GameState') -> None:
    ...
    if g.player == 1:                                      # L120
        ...
        for i in range(1, NUM_ARMIES + 1):                  # L123
    ...
    # ══════════════════════════════════════════════════════════════════
    for i in range(1, NUM_ARMIES + 1):                      # L138
        g.brray[i] = 999                                   # L139
    ...
    bub2(g, NUM_ARMIES)                                     # L159

    # ══════════════════════════════════════════════════════════════════
    #                    Begin Main Loop                      L163-331
    # ══════════════════════════════════════════════════════════════════
    for j in range(1, NUM_ARMIES + 1):                      # L163
        flag = 0                                           # L164
        ...
        elif (s_side == UNION and g.occupied[target] <= UNION_ARMY_END) or \
             (s_side == CONFEDERATE and g.occupied[target] > UNION_ARMY_END):  # L179: → friend
            # ── friend ──                                   L183-187
            s.color(11)                                    # L184
            clrbot(g)
...
def maptext(g: 'GameState') -> None:
    ...
    for k in range(1, NUM_CITIES + 1):                      # L411
        name = g.city[k]
        if not name:
            continue
        cx = g.cityx[k]
...
def usa(g: 'GameState') -> None:
    ...
    if g.player == 3:
        ...
        for i in range(1, NUM_ARMIES + 1):                  # L803
            if g.armyloc[i] > 0 and g.armymove[i] > 0:
                icon(g, g.armyloc[i], g.armymove[i], 1)
