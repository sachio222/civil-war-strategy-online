...
def _city_report(g: 'GameState') -> None:
    ...
    s.color(4)                                              # L98
    s.print_text(
        f"{g.force[0]:10s} {NUM_CITIES - g.control[1] - g.control[2]:4}"
    )
...
def _force_summary(g: 'GameState') -> None:
    ...
    s = g.screen
    summ = [0] * (NUM_CITIES + 1)                            # L132 (DIM summ(40))

    for k in range(1, NUM_CITIES + 1):                       # L132
        if g.occupied[k] > 0:                               # L133
            for j in range(1, NUM_ARMIES + 1):              # L134
    ...
    for k in range(1, NUM_CITIES + 1):                       # L141
        if summ[k] > 0:                                     # L142
            c = 9                                           # L143
