...
from cws_globals import UNION, CONFEDERATE, UNION_ARMY_END, NUM_CITIES, strong as _strong
...
def recruit(g: 'GameState', who: int) -> None:
    ...
    if who == g.side and g.rflag > 0:
        ...
        for i in range(1, NUM_CITIES + 1):                   # L9
            if g.control[who] > 0:                          # L10
                pct = 0.6 * max_cities / g.control[who]
            else:
                pct = 0.3
            if g.size == 0 and i > UNION_ARMY_END:           # L11
                pct = 0.3
