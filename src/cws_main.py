...
from cws_globals import (
    UNION, CONFEDERATE, VERSION,
    NUM_ARMIES, NUM_CITIES, UNION_ARMY_END,
)

DEFAULT_SERVER = "localhost:1861"
...
def _newgame_init(g: 'GameState', replay: int) -> None:
    ...
    g.pcode = 0
    g.rflag = 0
    for k in range(1, NUM_ARMIES + 1):                       # L10-16
        g.armysize[k] = 0
        g.armyloc[k] = 0
        g.armymove[k] = 0
        g.armylead[k] = 0
...
def _newmonth(g: 'GameState') -> bool:
    ...
    for i in range(1, NUM_CITIES + 1):                       # L150-153
        if g.cityp[i] > 0:                                 # L151
            x = g.cityp[i]
            g.control[x] += 1
    ...
    if g.pcode > 0:                                         # L165
        for k in range(1, NUM_ARMIES + 1):                   # L166
            g.armyloc[k] = 0
...
def _utility_menu(g: 'GameState') -> None:
    ...
    while True:                                             # utile:
        ...
        if g.choose == 1:                                   # Swap Sides (L420)
            ...
            for k in range(1, NUM_CITIES + 1):               # L574
                occupy(g, k)
            ...
            if random.random() > 0.7:                       # L584
                g.capcity[2] = 25
            for k in range(1, NUM_ARMIES + 1):               # L586
...
def _show_event_replay(g: 'GameState') -> None:
    ...
    for evt in events:
        ...
        # ──────────── Battle ────────────
            ...
            y = 68
            clrrite(g)
            c = 9 if evt["atk_id"] <= UNION_ARMY_END else 7
        ...
        # ──────────── Surrender / Crushed ────────────
            ...
            if g.noise > 1:
                loser_side = UNION if aid <= UNION_ARMY_END else CONFEDERATE
...
def _online_setup(g: 'GameState') -> str:
    ...
    if g.choose == 1:
        # ── Create Game ──
        server = _text_input(g, "Server address?", DEFAULT_SERVER)
        if not server:
            return "cancel"
