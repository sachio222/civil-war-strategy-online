...
from cws_globals import (UNION, CONFEDERATE, UNION_ARMY_END, CONFED_ARMY_START,
                         FORT_COST, strong as _strong)
...
def battle(g: 'GameState', attack: int, defend: int) -> tuple:
    ...
    if attack < CONFED_ARMY_START and g.side == CONFEDERATE and g.difficult > 3:   # L27
        x = x + 2 * g.difficult - 6
    if attack > UNION_ARMY_END and g.side == UNION and g.difficult < 3:   # L28
        x = x + 6 - 2 * g.difficult
    s.locate(9, y)                                         # L29
    ...
    if defend < CONFED_ARMY_START and g.side == CONFEDERATE and g.difficult > 3:   # L65
        x1 = x1 + 2 * g.difficult - 6
    if defend > UNION_ARMY_END and g.side == UNION and g.difficult < 3:   # L66
    ...
    a_side = UNION if win <= UNION_ARMY_END else CONFEDERATE  # L121
    draw_victory_banner(
    ...
    if g.player == 3:
        g.event_log.append({
            ...
            "winner_side": UNION if win <= UNION_ARMY_END else CONFEDERATE,
    ...
    if g.player == 1:
        if (g.side == UNION and attack > UNION_ARMY_END) or (g.side == CONFEDERATE and attack < CONFED_ARMY_START):
            s.color(14)
            clrbot(g)
...
def capture(g: 'GameState', active: int, c: int, s_side: int, flag: int) -> None:
    ...
    scribe(g, a_str, 1)
    scr.update()
    if active < CONFED_ARMY_START and g.noise > 1:          # L285
        from cws_sound import qb_play
        qb_play("MNMFL16o2T120dd.dd.co1b.o2do3g.ab.bb.ag")
    if active > UNION_ARMY_END and g.noise > 1:             # L286
        from cws_sound import qb_play
...
def retreat(g: 'GameState', defend: int) -> int:
    ...
    if g.player == 1:
        if (g.side == UNION and defend > UNION_ARMY_END) or (g.side == CONFEDERATE and defend < CONFED_ARMY_START):
            return 0
