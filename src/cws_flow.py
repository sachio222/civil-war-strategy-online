...
from cws_globals import (UNION, CONFEDERATE,
                         UNION_ARMY_START, UNION_ARMY_END,
                         CONFED_ARMY_START, CONFED_ARMY_END,
                         NUM_ARMIES, NUM_CITIES)
...
def iterate(g: 'GameState') -> None:
    ...
    side_s = UNION                                           # L9
    for i in range(1, NUM_ARMIES + 1):                       # L10
        if g.armyloc[i] < 1:                                # rip
            continue
        if i > UNION_ARMY_END:                              # L11
            side_s = CONFEDERATE
    ...
    y = 0                                                   # L33
    for i in range(UNION_ARMY_START, UNION_ARMY_END + 1):
        y += 0.1 * g.armysize[i]
    x = 0                                                   # L34
    for i in range(CONFED_ARMY_START, CONFED_ARMY_END + 1):
        x += 0.1 * g.armysize[i]
    ...
    if who == UNION:
        ...
        if random.random() <= 0.2 and g.navysize[CONFEDERATE] <= 9:  # L118
            ...
            if g.navysize[CONFEDERATE] > 0 and g.navyloc[CONFEDERATE] != 99:   # L119
                ...
                for i in range(1, NUM_CITIES + 1):           # L121
        ...
        for k in range(UNION_ARMY_START, UNION_ARMY_END + 1):  # L156
        ...
        if random.random() > 0.5 and g.year > 1861:        # L216
            ...
            for i in range(CONFED_ARMY_START, CONFED_ARMY_END + 1):  # L218
        ...
        if random.random() > 0.5 and g.year > 1861:        # L220
            ...
            for i in range(UNION_ARMY_START, UNION_ARMY_END + 1):  # L222
...
def victor(g: 'GameState') -> None:
    ...
    x = 0                                                   # Union
    for j in range(UNION_ARMY_START, UNION_ARMY_END + 1):
        if g.armyloc[j] > 0:
            x += g.armysize[j]
    y = 0                                                   # Confederate
    for j in range(CONFED_ARMY_START, CONFED_ARMY_END + 1):
        if g.armyloc[j] > 0:
            y += g.armysize[j]
